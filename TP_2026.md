...
```bash
# Node 2 and Node 3
# Authorize root login through SSH
...
**Install the iSCSI Gateway**
```bash
sudo apt install -y ceph-iscsi tcmu-runner
```
**Setup RBD Pool:**
```bash
# Create pool for RBD
...
**Create the iSCSI Gatway Config**
```bash
...
create node1.micsi.local 192.168.16.141
create node2.micsi.local 192.168.16.140

```

**Add the Block Device to the iSCSI Gateway**
```bash
cd /disks
create pool=rbd image=disk_1 size=90G
```

**Add iSCSI Initiators (Clients)**
```bash
...
**Remove Unused Pools**
Remove pools not linked to a service

```bash
# Allow Pool deletion
sudo ceph config set mon mon_allow_pool_delete true

sudo ceph osd pool delete ec-pool ec-pool --yes-i-really-really-mean-it
